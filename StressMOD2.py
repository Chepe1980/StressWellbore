...
with st.sidebar:
    ...
    if las:
        try:
            ...
            default_depth_range = [float(depth_data[0]), float(depth_data[-1])]
        except:
...
if las:
    ...
    if results:
        X, Y, Z, R, Theta, Depth, hoop_stress_fd, sigma_H, sigma_h, Pp = results
        ...
        st.subheader("Stress Data Overview")
        ...
        st.markdown("""
        ### Interpretation Guide:
        - All values are in psi (pounds per square inch)
        - **3D Wellbore Stress**: Stress distribution on wellbore surface
        - **Stress Concentration**: Areas exceeding threshold stress (potential failure zones)
        - **Circumferential Plot**: Stress variation around wellbore (0°=σH direction)
        - **Radial Decay**: Stress decreases with distance from wellbore
